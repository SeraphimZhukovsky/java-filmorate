...
public class FilmDbStorage implements FilmStorage {
    ...
    public Optional<Film> getFilm(int id) {
        ...
        try {
            Film film = jdbcTemplate.queryForObject(sql, (rs, rowNum) -> mapRowToFilm(rs), id);
            if (film != null) {
    ...
    public List<Film> getPopularFilms(int count) {
        String sql = "SELECT f.*, m.name AS mpa_name, COUNT(fl.user_id) AS likes_count " +
                ...
                "GROUP BY f.id, m.name " +  // Добавили m.name в GROUP BY для совместимости с SQL-стандартами
